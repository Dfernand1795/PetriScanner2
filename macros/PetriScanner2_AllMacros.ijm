    ...
    ////////////////////////////////////////    GUI     /////////////////////////////////////////////////
macro  "PetriScanner-SelectExperiment" {
...
macro "PetriScanner-Headless" {
    
    run("Set Measurements...", "area mean stack standard center bounding integrated redirect=None decimal=9");
    
    //Recieves the command line option.
    RunString = getArgument();
    print("Command used to run this code:\n"+RunString);
    Params = split(RunString,"--");
    detect_t = 270;
    r_Avg_col_size   = 1.68;
    T_Type   = "Otsu";
    Rmv_Bkgrnd   = true;
    ...
    verbose  = true;
    watershed= true;
    Selected_dishes = newArray(1,1,1,1,1,1);
    ...
    for(i=0; i<Params.length; i++){
        ...
        if (Param[0] == "detect_t"){
            detect_t = parseInt(Param[1]);
        };
        if (Param[0] == "Avg_col_size"){
            r_Avg_col_size = parseFloat(Param[1]);
        };
        if (Param[0] == "T_Type"){
            T_Type = Param[1];
        };
        if (Param[0] == "Rmv_Bkgrnd"){
            Rmv_Bkgrnd = parseInt(Param[1]);
        };
        if (Param[0] == "Color_correction"){
            Color_correction = parseInt(Param[1]);
        };
        if (Param[0] == "verbose"){
            verbose = parseInt(Param[1]);
        };
        if (Param[0] == "watershed"){
            watershed = parseInt(Param[1]);
        };
        if (Param[0] == "Selected_dishes"){
            Selected_dishes = Param[1];
            Selected_dishes = split(Selected_dishes,"_");
        };
    };
...
macro  "PetriScanner-DetectBoard" {
    open();
    dpi = getWidth()/8.5;
    
    sf  = dpi/300;
    Lwr = ((990/2)*sf)*((990/2)*sf)*3.1415;
    Upp = ((1100/2)*sf)*((1100/2)*sf)*3.1415;
    
    msg = "Welcome to PetriScanner    :)\n \nPlease indicate if you are scanning a\n\nboard with light or dark background";
    Dialog.create("PetriScanner - Detectig Boards");
    Dialog.addMessage(msg);
    Dialog.addCheckbox("Black Background", true);
    Dialog.addString("Board name", "Board_");
    Dialog.show();
    Bkgrn  = Dialog.getCheckbox();
    Name_of_Board = Dialog.getString();

    Names = getDirectory("image");
    rename("Boards");
    run("8-bit");
    ...
    run("Convert to Mask");
    run("Invert");
    run("Set Measurements...", "center bounding redirect=None decimal=0");
    run("Analyze Particles...", "size="+Lwr+"-"+Upp+" show=Masks display clear include add in_situ");
    print(Names+Name_of_Board+".csv");
    //saveAs("Results", Names+Name_of_Board+".csv");
    print("DPI of board: "+dpi);
    headings = split(String.getResultsHeadings);
    print("X----Y");
    for (row=0; row<nResults; row++) {
        BX = getResult(headings[2],row);
        BY = getResult(headings[3],row);
        print(BX + "    "  + BY);
    };
    selectWindow("Log");
    saveAs("Text",  Names+Name_of_Board+".board");  
    selectWindow("Log");
    saveAs("Text",  Names+Name_of_Board+".board");  
    close("Log");
    close("Results");
...
PetriScanner_instalation = false;
